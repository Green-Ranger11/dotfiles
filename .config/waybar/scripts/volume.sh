...
toggle-mute() {
	...
	notify-send "$title: $(check-muted)" -i "$(get-icon)" -h "string:x-canonical-private-synchronous:$title"
}
...
set-volume() {
	...
	notify-send "$title: ${new_vol}%" -h int:value:$new_vol -i "$icon" -h "string:x-canonical-private-synchronous:$title"
}
